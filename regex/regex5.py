...
# train.xlsx 를 읽어서 성별대로 4개의 시트를 만들어 정보 삽입하기
# train_gender.xlsx 로 저장
# Mr(sheet 명은 남성), Miss(미혼여성), Mrs(기혼여성), Others(기타)

# 기존의 엑셀 파일 열기
train_wb = openpyxl.load_workbook("./regex/train.xlsx")
work_sheet = train_wb.active
...
for row in work_sheet.rows:
    ...
    # 정규식과 매치되는 문자열 추출
    data = pattern.findall(row[3].value) # [' Mr. ']
...
wb_report.append(['기혼여성',married_women_survived,married_women_unsurvived,survived_rate])
